...
*   **In-App Manual:** Open the official PICO-8 manual from the left toolbar without stopping or suspending the current PICO-8 process. The embedded manual view provides find, previous-match, and next-match controls for quickly locating reference content.
...
*   **Editor Shortcuts:** Outside Splore, show Code, Sprite, Map, SFX, and Music on the first row, with ESC, Tab, Save, and Run on the second row, in a two-row 5×2 grid beneath MENU. The editor-tab controls enter PICO-8's native editor when needed and navigate to the requested editor using PICO-8's native Alt+Left/Right shortcuts; Save writes and Run executes the currently loaded cartridge through PICO-8's native shortcuts. The controls provide feedback only while pressed, do not retain a selected state, remain available while editing, and hide only in Splore.
